...
class PipelineStep:
    ...
    def read_csv(self, path, sep):
        template = os.path.join(os.path.abspath(os.path.dirname(os.path.dirname(__file__))), path)
        return csv.reader(open(template, "r", encoding="utf-8"), delimiter=sep)

    def read_dict_csv(self, path, sep):
        template = os.path.join(os.path.abspath(os.path.dirname(os.path.dirname(__file__))), path)
        return csv.DictReader(open(template, "r", encoding='utf-8-sig'), delimiter=sep)
